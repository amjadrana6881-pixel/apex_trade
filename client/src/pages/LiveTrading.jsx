...
export default function LiveTrading() {
  ...
  // Max 3 minutes
  const [duration, setDuration] = useState(Math.min(180, initialDuration));
  ...
  // 2. Check for running background trade on load / refresh
  useEffect(() => {
    if (token) {
  ...
  const checkActiveRunningTrade = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/user/active-trade`, {
  ...
  // 3. Listen to WebSocket automated trade resolution
  useEffect(() => {
    if (socket) {
      const handleTradeResolved = (data) => {
        if (data && data.trade) {
      ...
      socket.on('trade:resolved', handleTradeResolved);
      return () => {
        socket.off('trade:resolved', handleTradeResolved);
      };
    }
  }, [socket]);
  ...
  const durations = [
    ...
    { label: '60 Sec (1m)', value: 60 },
    { label: '120 Sec (2m)', value: 120 },
    { label: '180 Sec (3m)', value: 180 },
  ];
  ...
  const fetchTradeHistory = async (page) => {
    try {
      const res = await fetch(`${API_BASE}/api/user/trades?page=${page}&pageSize=8`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleConfirmAndExecute = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/user/trade/start`, {
        method: 'POST',
        headers: {
  ...
  const resolveTrade = async (tradeId) => {
    try {
      const res = await fetch(`${API_BASE}/api/user/trade/${tradeId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-24 md:pb-8">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-3xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 shadow-2xs">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm font-extrabold text-slate-900 mt-0.5">
                {activeSignal.instrument} • {activeSignal.order_type} MARKET • {Math.floor(activeSignal.duration_seconds / 60)} Mins Duration
              </p>
            </div>
          </div>
          ...
          <button
            onClick={() => {
              ...
              setDuration(Math.min(180, activeSignal.duration_seconds));
              setShowConfirmModal(true);
            }}
      ...
      {/* Floating Minimized Running Trade Badge (When user closed the countdown modal) */}
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-3.5 sm:p-4 rounded-2xl shadow-lg flex items-center justify-between gap-3 animate-in slide-in-from-top-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-blue-100">Trade running in background... Click to view live seconds.</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-8 space-y-4 sm:space-y-6 order-1">
          ...
          {/* User Trade History Ledger (Desktop Position) */}
        ...
        <div className="lg:col-span-4 space-y-4 sm:space-y-6 order-2">
          <div className="bg-white border border-slate-200 rounded-3xl p-4 sm:p-7 shadow-xs space-y-4 sm:space-y-5">
            ...
            <div className="border-b border-slate-100 pb-3 flex justify-between items-center">
              <div>
                ...
                <p className="text-[11px] text-slate-500">Max duration 3 Mins • Full balance support</p>
              </div>
            ...
            <div className="space-y-1.5">
              <label className="block text-[11px] font-bold uppercase text-slate-400">Select Duration (Max 3 Min)</label>
            ...
            {isMatchingActiveSignal ? (
              ...
              <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-xl text-[11px] text-amber-800 leading-snug flex items-center gap-2">
                ...
                <span>Unscheduled off-signal trade: Subject to 100% market risk.</span>
              </div>
            )}
          ...
          {/* Trade History on Mobile */}
          <div className="block lg:hidden bg-white border border-slate-200 rounded-3xl p-4 shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-extrabold text-slate-900 flex items-center gap-1.5">
                <History className="w-4 h-4 text-blue-600" />
                <span>Trade History</span>
              </h2>
              <span className="text-[11px] text-slate-400">{historyMeta.total || 0} Trades</span>
            </div>

            <div className="space-y-2">
              {tradeHistory.slice(0, 4).map((t) => (
                <div key={t.id} className="p-2.5 bg-slate-50 rounded-xl flex items-center justify-between text-xs">
                  <div>
                    <span className="font-extrabold text-slate-900">{t.pair}</span>
                    <span className={`ml-2 px-1.5 py-0.2 rounded text-[9px] font-black ${t.type === 'BUY' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>
                      {t.type}
                    </span>
                  </div>
                  <div className="text-right font-mono font-bold">
                    <span className={t.result === 'WIN' ? 'text-emerald-600' : 'text-rose-600'}>
      ...
      {/* 2. ACTIVE TRADE COUNTDOWN MODAL (Minimizable & Closeable) */}
      {activeTrade && isCountdownModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-sm bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-2xl text-center space-y-5">
            ...
            <p className="text-xs text-slate-400">
              Live trade executing on server. You can minimize this modal or close your app safely.
            </p>
              ...
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Profit added to your available spot balance.
                  </p>
                </div>
              </>
            ) : (
