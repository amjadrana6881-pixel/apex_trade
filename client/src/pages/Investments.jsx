...
export default function Investments() {
      ...
      .then(data => {
        if (data.success) {
          ...
          if (data.data.length > 0) {
            ...
            setCalcPlan(data.data[0]);
            setInvestAmount(data.data[0].min_amount);
          }
        }
      })
      .catch(console.error);
  ...
  const calcDaily = calcPlan ? ((calcAmount * calcPlan.daily_roi) / 100).toFixed(2) : 0;
  const calcTotal = calcPlan ? (((calcAmount * calcPlan.daily_roi) / 100) * calcPlan.duration_days).toFixed(2) : 0;
