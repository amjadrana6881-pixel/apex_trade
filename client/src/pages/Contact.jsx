...
export default function Contact() {
  ...
  // Handle Socket Events for Live Chat
  useEffect(() => {
    if (socket) {
      const handleNewMsg = (newMsg) => {
        ...
        // If message is from admin, mark as seen
        if (newMsg.sender_role === 'admin') {
          fetch(`${API_BASE}/api/support/messages/mark-seen`, {
            method: 'POST',
      ...
      const handleMsgDeletedForEveryone = ({ id }) => {
      ...
      const handleMessagesSeen = ({ seenBy }) => {
        if (seenBy === 'admin') {
          setMessages((prev) => prev.map(m => m.sender_role === 'user' ? { ...m, is_seen: 1 } : m));
        }
      };

      socket.on('support:message', handleNewMsg);
      socket.on('support:message_updated', handleMsgUpdated);
      socket.on('support:message_deleted_for_everyone', handleMsgDeletedForEveryone);
      socket.on('support:messages_seen', handleMessagesSeen);

      return () => {
        socket.off('support:message', handleNewMsg);
        socket.off('support:message_updated', handleMsgUpdated);
        socket.off('support:message_deleted_for_everyone', handleMsgDeletedForEveryone);
        socket.off('support:messages_seen', handleMessagesSeen);
      };
    }
  }, [socket, token]);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/support/messages/${editingMsg.id}/edit`, {
        method: 'PUT',
        headers: {
  ...
  const handleDeleteMessage = async (mode) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/support/messages/${deletingMsg.id}?mode=${mode}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 pb-20 md:pb-8">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl shadow-xs overflow-hidden flex flex-col h-[560px] sm:h-[640px]">
                  ...
                  <div className="relative flex items-center gap-1.5 max-w-[88%] sm:max-w-[75%]">
                    ...
                    >
                      ...
                      {m.image_url && !isDeleted && (
                        <div 
                          onClick={() => setZoomedImage(`${API_BASE}${m.image_url}`)}
                      ...
                      <div className="flex items-center justify-end gap-1 mt-1">
                        ...
                        {/* Read Receipts for User Messages (Single Tick vs Double Blue Ticks) */}
                        {isMe && !isDeleted && (
                          <span title={m.is_seen === 1 ? 'Seen by Support Officer' : 'Delivered to Server'}>
                            {m.is_seen === 1 ? (
        ...
        {/* Selected Image Thumbnail Bar (Before sending) */}
        {imagePreview && (
      ...
      {/* 5. DELETE MESSAGE MODAL (Delete for Me & Delete for Everyone) */}
      {deletingMsg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
