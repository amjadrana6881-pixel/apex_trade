import { connectToDatabase } from './db.js';
import User from '../models/User.js';
import Trade from '../models/Trade.js';
import Signal from '../models/Signal.js';
import Transaction from '../models/Transaction.js';
import SystemSetting from '../models/SystemSetting.js';
import TradingPair from '../models/TradingPair.js';
import { sendPushToUser } from './fcm.js';

/**
 * Resolves a single pending trade record using institutional business rules.
 */
export async function resolveTradeRecord(trade) {
  try {
    await connectToDatabase();

    const tradeId = trade._id || trade.id;

    // ATOMIC LOCK: Atomically claim the trade by moving from 'PENDING' to 'RESOLVING'
    // Guarantees only one concurrent request/worker can ever process this trade
    const currentTrade = await Trade.findOneAndUpdate(
      { _id: tradeId, status: 'PENDING' },
      { $set: { status: 'RESOLVING' } },
      { returnDocument: 'after' }
    );

    if (!currentTrade) {
      // Trade is already RESOLVED, RESOLVING, STOPPED, or doesn't exist
      return await Trade.findById(tradeId);
    }

    // Secondary Safety Guard: Check if a win/loss transaction already exists for this trade
    const existingTx = await Transaction.findOne({
      reference_id: tradeId.toString(),
      type: { $in: ['TRADE_WIN', 'TRADE_LOSS', 'TRADE_EARLY_EXIT'] }
    });

    if (existingTx) {
      console.warn(`[RESOLVE GUARD] Settlement transaction already exists for trade ${tradeId}. Aborting duplicate payout.`);
      currentTrade.status = 'RESOLVED';
      await currentTrade.save();
      return currentTrade;
    }

    const user = await User.findById(currentTrade.user_id);
    if (!user) {
      currentTrade.status = 'PENDING';
      await currentTrade.save();
      return null;
    }

    let isWin = false;

    // 1. Admin Individual User Override takes highest priority
    if (user.trade_mode === 'FORCE_WIN') {
      isWin = true;
    } else if (user.trade_mode === 'FORCE_LOSS') {
      isWin = false;
    } 
    // 2. Official Daily Signal Trade: Guaranteed Win (or Planned Signal Outcome)
    else if (currentTrade.is_signal_trade && currentTrade.signal_id) {
      const signal = await Signal.findById(currentTrade.signal_id);
      if (signal && signal.outcome === 'LOSS') {
        isWin = false; // Planned loss day set by Admin
      } else {
        isWin = true;  // Official Signal trade always WINS
      }
    } 
    // 3. Unscheduled Off-Signal Trade: Guaranteed Loss for all trades outside signal
    else {
      isWin = false; // Strict House Rule: Any trade placed without/outside official signal is guaranteed LOSS
    }

    // Exit price calculation: Adjust price dynamically based on win or loss
    const priceDelta = currentTrade.entry_price * 0.0018;
    let exitPrice = currentTrade.entry_price;

    if (currentTrade.type === 'BUY') {
      exitPrice = isWin ? (currentTrade.entry_price + priceDelta) : (currentTrade.entry_price - priceDelta);
    } else { // SELL
      exitPrice = isWin ? (currentTrade.entry_price - priceDelta) : (currentTrade.entry_price + priceDelta);
    }

    let profitAmount = 0;
    let lossAmount = 0;
    let refundAmount = 0;
    let lossPct = 0;

    if (isWin) {
      profitAmount = Number((currentTrade.amount * (currentTrade.payout_rate / 100)).toFixed(2));
    } else {
      // 1. Check if Admin configured a specific planned loss percentage on the Signal
      let customLossRate = null;
      if (currentTrade.signal_id) {
        const signal = await Signal.findById(currentTrade.signal_id);
        if (signal && signal.outcome === 'LOSS' && signal.loss_percentage > 0) {
          customLossRate = signal.loss_percentage;
        }
      }

      if (customLossRate) {
        lossPct = Number(customLossRate.toFixed(2));
      } else {
        // Dynamic 3.00% to 5.00% capped loss for non-winning / unscheduled / off-time trades
        lossPct = Number((3.0 + Math.random() * 2.0).toFixed(2)); // e.g. 3.45%, 4.12%, etc.
      }

      lossAmount = Number(((currentTrade.amount * lossPct) / 100).toFixed(2));
      refundAmount = Number((currentTrade.amount - lossAmount).toFixed(2));
      profitAmount = -lossAmount;
    }

    const result = isWin ? 'WIN' : 'LOSS';
    const walletUsed = currentTrade.wallet_amount_used !== undefined ? Number(currentTrade.wallet_amount_used || 0) : Number(currentTrade.amount || 0);
    const stakedUsed = Number(currentTrade.staked_amount_used || 0);

    // If win, credit wallet principal used + entire profit into wallet_balance (staked principal remains in investment_balance)
    if (isWin) {
      const returnTotal = Number((walletUsed + profitAmount).toFixed(2));
      user.wallet_balance = Number((user.wallet_balance + returnTotal).toFixed(2));
      user.tradeable_amount = user.wallet_balance;
      await user.save();

      await Transaction.create({
        user_id: user._id,
        type: 'TRADE_WIN',
        amount: returnTotal,
        description: `Trade WON on ${currentTrade.pair} (+ $${profitAmount.toFixed(2)} profit${currentTrade.is_investment_boosted ? ' [VIP Yield Boost]' : ''}${stakedUsed > 0 ? ` on $${currentTrade.amount.toFixed(2)} capital` : ''})`,
        reference_id: currentTrade._id.toString(),
        status: 'COMPLETED'
      });

      // Send lock-screen push notification
      try {
        await sendPushToUser(user._id, {
          title: `💰 Option Trade Won (+ $${profitAmount.toFixed(2)})${currentTrade.is_investment_boosted ? ' [VIP Boost]' : ''}!`,
          body: `Congratulations! Your ${currentTrade.type} contract on ${currentTrade.pair} won${currentTrade.is_investment_boosted ? ' with VIP Investment Boost' : ''}. Profit credited to your wallet.`,
          data: { type: 'TRADE_RESULT', result: 'WIN', profit: String(profitAmount), target_url: '/trading' }
        });
      } catch (pushErr) {
        console.error('Trade win push error:', pushErr);
      }

      // Distribute 3-Tier Daily Trade Profit Commissions (10% Tier 1, 5% Tier 2, 2% Tier 3)
      if (user.referred_by && profitAmount > 0) {
        try {
          await distributeTradeProfitCommission(user, profitAmount, currentTrade._id.toString(), currentTrade.pair);
        } catch (e) {
          console.error('Error distributing trade profit commission:', e);
        }
      }
    } else {
      // Refund protected capital back into wallet and adjust staked capital if needed
      const walletRefund = walletUsed > 0 ? Number((walletUsed * (1 - lossPct / 100)).toFixed(2)) : 0;
      const stakedRefund = stakedUsed > 0 ? Number((stakedUsed * (1 - lossPct / 100)).toFixed(2)) : 0;

      if (walletRefund > 0) {
        user.wallet_balance = Number((user.wallet_balance + walletRefund).toFixed(2));
        user.tradeable_amount = user.wallet_balance;
      }
      if (stakedUsed > 0 && stakedRefund < stakedUsed) {
        const stakedLoss = Number((stakedUsed - stakedRefund).toFixed(2));
        user.investment_balance = Math.max(0, Number(((user.investment_balance || 0) - stakedLoss).toFixed(2)));
      }
      await user.save();

      // Log Trade Loss Transaction with protected capital return details
      await Transaction.create({
        user_id: user._id,
        type: 'TRADE_LOSS',
        amount: -lossAmount,
        description: `Trade EXPIRED on ${currentTrade.pair} (-$${lossAmount.toFixed(2)} [${lossPct}% loss], +$${refundAmount.toFixed(2)} protected capital preserved)`,
        reference_id: currentTrade._id.toString(),
        status: 'COMPLETED'
      });

      // Send push notification on trade loss
      try {
        await sendPushToUser(user._id, {
          title: `📉 Trade Completed (- $${lossAmount.toFixed(2)})`,
          body: `Your trade expired with 3-5% risk protection. $${refundAmount.toFixed(2)} capital was preserved and returned to your balance.`,
          data: { type: 'TRADE_RESULT', result: 'LOSS', loss: String(lossAmount), refunded: String(refundAmount), target_url: '/trading' }
        });
      } catch (pushErr) {
        console.error('Trade loss push error:', pushErr);
      }
    }

    // Update trade record to RESOLVED
    currentTrade.status = 'RESOLVED';
    currentTrade.result = result;
    currentTrade.profit = isWin ? profitAmount : -lossAmount;
    currentTrade.refunded_amount = isWin ? Number((currentTrade.amount + profitAmount).toFixed(2)) : refundAmount;
    currentTrade.penalty_percentage = isWin ? 0 : lossPct;
    currentTrade.exit_price = exitPrice;
    await currentTrade.save();

    return currentTrade;
  } catch (err) {
    console.error('Error resolving trade record:', err);
    return null;
  }
}

/**
 * Stops an active/pending trade prematurely (Early Exit / Stop Trade).
 * Calculates a dynamic 3% to 5% early settlement fee, refunds the remaining 95% to 97% to user's wallet.
 */
export async function stopTradeEarly(tradeId, userId) {
  try {
    await connectToDatabase();

    // ATOMIC LOCK: Atomically claim the trade by moving from 'PENDING' to 'STOPPING'
    const trade = await Trade.findOneAndUpdate(
      { _id: tradeId, user_id: userId, status: 'PENDING' },
      { $set: { status: 'STOPPING' } },
      { returnDocument: 'after' }
    );
    if (!trade) {
      return { success: false, error: 'Active trade not found or already processed.' };
    }

    // Secondary Safety Guard
    const existingTx = await Transaction.findOne({
      reference_id: tradeId.toString(),
      type: { $in: ['TRADE_WIN', 'TRADE_LOSS', 'TRADE_EARLY_EXIT'] }
    });
    if (existingTx) {
      trade.status = 'STOPPED';
      await trade.save();
      return { success: false, error: 'Trade settlement already completed.' };
    }

    const user = await User.findById(userId);
    if (!user) {
      trade.status = 'PENDING';
      await trade.save();
      return { success: false, error: 'User account not found.' };
    }

    // Dynamic penalty percentage between 3.00% and 5.00%
    const penaltyPct = Number((3.0 + Math.random() * 2.0).toFixed(2)); // e.g. 3.45%, 4.20%, etc.
    const penaltyFee = Number(((trade.amount * penaltyPct) / 100).toFixed(2));
    const refundAmount = Number((trade.amount - penaltyFee).toFixed(2));

    const walletUsed = trade.wallet_amount_used !== undefined ? Number(trade.wallet_amount_used || 0) : Number(trade.amount || 0);
    const stakedUsed = Number(trade.staked_amount_used || 0);

    const walletRefund = walletUsed > 0 ? Number((walletUsed * (1 - penaltyPct / 100)).toFixed(2)) : 0;
    const stakedRefund = stakedUsed > 0 ? Number((stakedUsed * (1 - penaltyPct / 100)).toFixed(2)) : 0;

    if (walletRefund > 0) {
      user.wallet_balance = Number((user.wallet_balance + walletRefund).toFixed(2));
      user.tradeable_amount = user.wallet_balance;
    }
    if (stakedUsed > 0 && stakedRefund < stakedUsed) {
      const stakedPenalty = Number((stakedUsed - stakedRefund).toFixed(2));
      user.investment_balance = Math.max(0, Number(((user.investment_balance || 0) - stakedPenalty).toFixed(2)));
    }
    await user.save();

    // Log transaction
    await Transaction.create({
      user_id: user._id,
      type: 'TRADE_EARLY_EXIT',
      amount: refundAmount,
      description: `Early Close Option on ${trade.pair} (-$${penaltyFee.toFixed(2)} [${penaltyPct}% fee], +$${refundAmount.toFixed(2)} refunded to wallet)`,
      reference_id: trade._id.toString(),
      status: 'COMPLETED'
    });

    // Mark trade as STOPPED / RESOLVED
    trade.status = 'STOPPED';
    trade.result = 'STOPPED';
    trade.profit = -penaltyFee;
    trade.early_stop_fee = penaltyFee;
    trade.refunded_amount = refundAmount;
    trade.penalty_percentage = penaltyPct;
    trade.stopped_early = true;
    trade.exit_price = trade.entry_price;
    await trade.save();

    // Notify User of Early Stop & Refund
    try {
      await sendPushToUser(user._id, {
        title: `🛑 Trade Stopped Early (+ $${refundAmount.toFixed(2)} Refunded)`,
        body: `Your trade on ${trade.pair} was closed early. $${refundAmount.toFixed(2)} refunded (${penaltyPct}% exit fee).`,
        data: { type: 'TRADE_EARLY_EXIT', trade_id: trade._id.toString(), target_url: '/trading' }
      });
    } catch (pushErr) {
      console.error('Early exit push error:', pushErr);
    }

    return {
      success: true,
      trade,
      penaltyPct,
      penaltyFee,
      refundAmount,
      updatedBalance: user.wallet_balance
    };
  } catch (err) {
    console.error('Error stopping trade early:', err);
    return { success: false, error: 'Failed to stop trade early.' };
  }
}

/**
 * Resolves all pending trades whose resolution time has passed.
 */
export async function processExpiredTrades() {
  try {
    await connectToDatabase();
    const now = new Date();
    const expiredTrades = await Trade.find({
      status: 'PENDING',
      resolves_at: { $lte: now }
    });

    for (const trade of expiredTrades) {
      await resolveTradeRecord(trade);
    }
  } catch (err) {
    console.error('Error in processExpiredTrades worker:', err);
  }
}

/**
 * Updates prices with realistic real-time micro fluctuations
 */
export async function tickMarketPrices() {
  try {
    await connectToDatabase();
    const pairs = await TradingPair.find({ is_active: true });

    for (const pair of pairs) {
      let pct = (Math.random() * 0.004) - 0.0019; // -0.19% to +0.21%
      if (pair.category === 'Crypto') {
        pct = (Math.random() * 0.008) - 0.0038;
      }

      let newPrice = pair.current_price * (1 + pct);
      if (pair.category === 'Forex') {
        newPrice = Number(newPrice.toFixed(4));
      } else if (newPrice > 1000) {
        newPrice = Number(newPrice.toFixed(2));
      } else if (newPrice > 10) {
        newPrice = Number(newPrice.toFixed(2));
      } else {
        newPrice = Number(newPrice.toFixed(4));
      }

      let newChange = Number((pair.change + (pct * 10)).toFixed(2));
      if (Math.abs(newChange) > 15) {
        newChange = Number((pct * 10).toFixed(2));
      }

      pair.current_price = newPrice;
      pair.change = newChange;
      await pair.save();
    }
  } catch (err) {
    console.error('Error in tickMarketPrices:', err);
  }
}

/**
 * Distributes 3-Tier Daily Trade Profit Commission to sponsors when a downline member wins a trade.
 * Tier 1: 10% of profit
 * Tier 2: 5% of profit
 * Tier 3: 2% of profit
 */
export async function distributeTradeProfitCommission(traderUser, profitAmount, tradeId, pair = '') {
  try {
    if (!traderUser || !traderUser.referred_by || profitAmount <= 0) return;

    await connectToDatabase();

    const lvl1Setting = await SystemSetting.findOne({ key: 'referral_trade_lvl1_pct' });
    const lvl2Setting = await SystemSetting.findOne({ key: 'referral_trade_lvl2_pct' });
    const lvl3Setting = await SystemSetting.findOne({ key: 'referral_trade_lvl3_pct' });

    const lvl1Pct = Number(lvl1Setting?.value || 10);
    const lvl2Pct = Number(lvl2Setting?.value || 5);
    const lvl3Pct = Number(lvl3Setting?.value || 2);

    // Tier 1 sponsor
    const u1 = await User.findOne({ referral_code: traderUser.referred_by });
    if (u1) {
      const bonus1 = Number(((profitAmount * lvl1Pct) / 100).toFixed(2));
      if (bonus1 > 0) {
        u1.wallet_balance = Number((u1.wallet_balance + bonus1).toFixed(2));
        u1.tradeable_amount = u1.wallet_balance;
        await u1.save();

        await Transaction.create({
          user_id: u1._id,
          type: 'REFERRAL_BONUS',
          amount: bonus1,
          description: `Tier 1 Trade Profit Commission from ${traderUser.name} ($${profitAmount.toFixed(2)} profit on ${pair || 'Trade'})`,
          reference_id: tradeId,
          status: 'COMPLETED'
        });

        // Push notification to sponsor
        try {
          await sendPushToUser(u1._id, {
            title: `💰 Team Trade Commission (+ $${bonus1.toFixed(2)})`,
            body: `You earned ${lvl1Pct}% Tier 1 commission from ${traderUser.name}'s winning trade profit ($${profitAmount.toFixed(2)})!`,
            data: { type: 'REFERRAL_COMMISSION', amount: String(bonus1), target_url: '/referral-tree' }
          });
        } catch (pushErr) {}
      }

      // Tier 2 sponsor
      if (u1.referred_by) {
        const u2 = await User.findOne({ referral_code: u1.referred_by });
        if (u2) {
          const bonus2 = Number(((profitAmount * lvl2Pct) / 100).toFixed(2));
          if (bonus2 > 0) {
            u2.wallet_balance = Number((u2.wallet_balance + bonus2).toFixed(2));
            u2.tradeable_amount = u2.wallet_balance;
            await u2.save();

            await Transaction.create({
              user_id: u2._id,
              type: 'REFERRAL_BONUS',
              amount: bonus2,
              description: `Tier 2 Trade Profit Commission from ${traderUser.name} ($${profitAmount.toFixed(2)} profit on ${pair || 'Trade'})`,
              reference_id: tradeId,
              status: 'COMPLETED'
            });

            try {
              await sendPushToUser(u2._id, {
                title: `💰 Team Trade Commission (+ $${bonus2.toFixed(2)})`,
                body: `You earned ${lvl2Pct}% Tier 2 commission from ${traderUser.name}'s winning trade profit ($${profitAmount.toFixed(2)})!`,
                data: { type: 'REFERRAL_COMMISSION', amount: String(bonus2), target_url: '/referral-tree' }
              });
            } catch (pushErr) {}
          }

          // Tier 3 sponsor
          if (u2.referred_by) {
            const u3 = await User.findOne({ referral_code: u2.referred_by });
            if (u3) {
              const bonus3 = Number(((profitAmount * lvl3Pct) / 100).toFixed(2));
              if (bonus3 > 0) {
                u3.wallet_balance = Number((u3.wallet_balance + bonus3).toFixed(2));
                u3.tradeable_amount = u3.wallet_balance;
                await u3.save();

                await Transaction.create({
                  user_id: u3._id,
                  type: 'REFERRAL_BONUS',
                  amount: bonus3,
                  description: `Tier 3 Trade Profit Commission from ${traderUser.name} ($${profitAmount.toFixed(2)} profit on ${pair || 'Trade'})`,
                  reference_id: tradeId,
                  status: 'COMPLETED'
                });

                try {
                  await sendPushToUser(u3._id, {
                    title: `💰 Team Trade Commission (+ $${bonus3.toFixed(2)})`,
                    body: `You earned ${lvl3Pct}% Tier 3 commission from ${traderUser.name}'s winning trade profit ($${profitAmount.toFixed(2)})!`,
                    data: { type: 'REFERRAL_COMMISSION', amount: String(bonus3), target_url: '/referral-tree' }
                  });
                } catch (pushErr) {}
              }
            }
          }
        }
      }
    }
  } catch (err) {
    console.error('Trade profit commission distribution error:', err);
  }
}

