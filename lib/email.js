...
export function getTransporter() {
  ...
  // Gmail App Passwords (16 chars) can have spaces between groups; remove whitespace for SMTP authentication
  const emailPass = rawPass.replace(/\s+/g, '');
  const smtpHost = process.env.SMTP_HOST || (emailUser.endsWith('@gmail.com') ? 'smtp.gmail.com' : 'smtp.gmail.com');
...
/**
 * Generate branded HTML email template for ApexTrader OTP
 */
export function generateOtpHtml({ code, type, recipientEmail }) {
  const isRegister = type === 'REGISTER';
  const title = isRegister ? 'Verify Your ApexTrader Account' : 'Reset Your ApexTrader Password';
  const actionText = isRegister 
    ? 'Thank you for choosing ApexTrader Pro. Use the verification code below to complete your registration:'
                ...
                <tr>
                  ...
                  <td style="padding-left: 12px; text-align: left;">
                    <span style="font-size: 22px; font-weight: 900; color: #ffffff; letter-spacing: -0.5px; display: inline-block;">
                      ApexTrader <span style="background-color: rgba(37, 99, 235, 0.3); color: #93c5fd; font-size: 11px; padding: 2px 8px; border-radius: 9999px; border: 1px solid rgba(147, 197, 253, 0.3); font-weight: 800; vertical-align: middle;">PRO</span>
                    </span>
            ...
            <td style="padding: 36px 32px 28px;">
              ...
              <div style="background-color: rgba(30, 41, 59, 0.6); border-radius: 12px; border-left: 4px solid #3b82f6; padding: 14px 16px; margin: 24px 0;">
                <p style="margin: 0; font-size: 12px; color: #cbd5e1; line-height: 1.5;">
                  🔒 <strong>Security Warning:</strong> Never share this OTP code with anyone. ApexTrader staff will never ask for your verification code or account password.
                </p>
              </div>
          ...
          <tr>
            <td style="padding: 20px 32px 28px; background-color: #0b0f19; border-top: 1px solid #1f2937; text-align: center;">
              <p style="margin: 0 0 6px; font-size: 11px; color: #64748b;">
                ApexTrader Inc. • Global Institutional Derivatives & Option Platform
              </p>
 ...
 */
export async function sendOtpEmail({ to, code, type = 'REGISTER' }) {
  const cleanEmail = (to || '').toLowerCase().trim();
  const isRegister = type === 'REGISTER';
  const subject = isRegister 
    ? `[ApexTrader] Your Registration Verification Code: ${code}`
    : `[ApexTrader] Password Reset Verification Code: ${code}`;

  const transporter = getTransporter();

  if (!transporter) {
    console.warn(`⚠️ [EMAIL SERVICE] SMTP credentials not configured. OTP for ${cleanEmail}: ${code}`);
    return {
      success: true,
      emailSent: false,
  ...
  try {
    const fromAddress = process.env.SMTP_FROM || process.env.EMAIL_FROM || `"ApexTrader Security" <${process.env.EMAIL_USER || process.env.SMTP_USER}>`;
    ...
    const info = await transporter.sendMail({
      ...
      text: `Your ApexTrader ${isRegister ? 'Registration' : 'Password Reset'} verification code is: ${code}. This code is valid for 10 minutes. Do not share it with anyone.`
    });
