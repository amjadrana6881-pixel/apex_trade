...
import TradingPairCard from '@/app/components/TradingPairCard';
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Financial Markets & Assets</h2>
            <p className="text-xs text-slate-500">Live prices streamed via institutional liquidity providers.</p>
          </div>
        ...
        {loadingPairs && pairs.length === 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3.5 sm:gap-4">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((n) => (
              <div key={n} className="h-44 bg-slate-200/60 rounded-3xl animate-pulse"></div>
            ))}
          </div>
        ) : activeCategory === 'All' ? (
          <div className="space-y-6">
            {['Commodities', 'Forex', 'Crypto', 'Stocks'].map((cat) => {
              const catPairs = pairs.filter(p => p.category?.toLowerCase() === cat.toLowerCase());
                ...
                <div key={cat} className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="text-base sm:text-lg font-black text-slate-900 tracking-tight flex items-center gap-1.5">
                      <span>{cat}</span>
                      <span className="text-xs font-semibold text-slate-500">({catPairs.length} pairs)</span>
                    </h3>
                  </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-base sm:text-lg font-black text-slate-900 tracking-tight flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-slate-500">({filteredPairs.length} pairs)</span>
              </h3>
            </div>
